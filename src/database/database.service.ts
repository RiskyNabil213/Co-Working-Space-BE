...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  private async initDatabase() {
    try {
      ...
      // Seed default maker if not exists
      ...
      if (!defaultMaker) {
        ...
        // Seed default Admin Space for this maker
        const adminPass = await bcrypt.hash('Admin123!', 10);
        const userAdmin = await this.run(
      ...
      // Check if sample reservations exist for makerId
      if (makerId) {
        ...
        if (resCount && resCount.count === 0) {
          ...
          if (firstMember && space1 && space2) {
            // 1. Pending Approval Reservation (so Admin has something to approve!)
            await this.run(
