...
let AdminService = class AdminService {
    ...
    normalizeStatus(status) {
        ...
        if (s === 'pending' || s === 'belum_dikonfirmasi')
            return 'belum_dikonfirm';
        if (s === 'approved')
            return 'disetujui';
        if (s === 'active')
    ...
    async createSpace(user, body) {
        ...
        if (!ownerId) {
            const owner = await this.db.get('SELECT id FROM space_owners WHERE user_id = ?', [user.id]);
            if (owner) {
                ownerId = owner.id;
            }
            else {
                const firstOwner = await this.db.get('SELECT so.id FROM space_owners so JOIN users u ON so.user_id = u.id WHERE u.maker_id = ? LIMIT 1', [user.maker_id]);
                ownerId = firstOwner ? firstOwner.id : 1;
            }
        }
        const now = new Date().toISOString();
        const res = await this.db.run(`INSERT INTO spaces (
    ...
    async updateSpace(user, id, body) {
        const space = await this.db.get('SELECT * FROM spaces WHERE id = ? AND maker_id = ?', [id, user.maker_id]);
        if (!space) {
        ...
        const { nama_space, harga_per_jam, tipe, kapasitas, deskripsi, foto } = body;
        const now = new Date().toISOString();
        await this.db.run(`UPDATE spaces SET
       ...
       WHERE id = ? AND maker_id = ?`, [
        ...
        const updated = await this.db.get('SELECT * FROM spaces WHERE id = ?', [id]);
        return {
            success: true,
    ...
    async deleteSpace(user, id) {
        ...
        await this.db.run('DELETE FROM spaces WHERE id = ? AND maker_id = ?', [id, user.maker_id]);
        return {
            success: true,
    ...
    async getCoworkingProfile(user) {
        let owner = await this.db.get(`SELECT so.*, u.username
       FROM space_owners so
       JOIN users u ON so.user_id = u.id
       WHERE u.maker_id = ? AND u.role = 'admin_space'
       LIMIT 1`, [user.maker_id]);
        if (!owner) {
            owner = {
    ...
    async updateCoworkingProfile(user, body) {
        ...
        const now = new Date().toISOString();
        const owner = await this.db.get(`SELECT so.* FROM space_owners so
       JOIN users u ON so.user_id = u.id
       WHERE u.maker_id = ? AND u.role = 'admin_space'
       LIMIT 1`, [user.maker_id]);
        if (owner) {
            await this.db.run(`UPDATE space_owners SET
         ...
         WHERE id = ?`, [
            ...
            await this.db.run(`INSERT INTO space_owners (user_id, nama_coworking, nama_pemilik, alamat, telp, deskripsi_fasilitas, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                user.id,
                nama_coworking || 'UHUB Premium Coworking',
                nama_pemilik || user.username || 'Admin Space',
    ...
    async getDiscounts(user) {
        const discounts = await this.db.all('SELECT * FROM diskons WHERE maker_id = ? ORDER BY id DESC', [user.maker_id]);
        return {
            success: true,
    ...
    async createDiscount(user, body) {
        ...
        const existing = await this.db.get('SELECT id FROM diskons WHERE UPPER(nama_diskon) = UPPER(?) AND maker_id = ?', [nama_diskon, user.maker_id]);
        if (existing) {
            throw new common_1.BadRequestException({
                success: false,
        ...
        const now = new Date().toISOString();
        const res = await this.db.run(`INSERT INTO diskons (maker_id, nama_diskon, persentase_diskon, tanggal_awal, tanggal_akhir, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [
            user.maker_id,
    ...
    async updateDiscount(user, id, body) {
        const diskon = await this.db.get('SELECT * FROM diskons WHERE id = ? AND maker_id = ?', [id, user.maker_id]);
        if (!diskon) {
            throw new common_1.NotFoundException({
                success: false,
        ...
        const { nama_diskon, persentase_diskon, tanggal_awal, tanggal_akhir } = body;
        const now = new Date().toISOString();
        await this.db.run(`UPDATE diskons SET
    ...
    async deleteDiscount(user, id) {
        ...
        await this.db.run('DELETE FROM diskons WHERE id = ? AND maker_id = ?', [id, user.maker_id]);
        return {
            success: true,
            message: 'Diskon berhasil dihapus.',
        };
    }
    async getAllReservations(user, queryParams) {
    ...
    async updateReservationStatus(user, id, body) {
        ...
        const updateParams = [normalizedStatus, now];
        ...
        await this.db.run(`UPDATE reservasis SET ${updateFields} WHERE id = ? AND maker_id = ?`, updateParams);
        return {
            success: true,
    ...
    async checkIn(user, id) {
        const reservation = await this.db.get(`SELECT r.*, s.nama_space, m.nama_member
       FROM reservasis r
       JOIN spaces s ON r.id_space = s.id
       JOIN members m ON r.id_member = m.id
       WHERE r.id = ? AND r.maker_id = ?`, [id, user.maker_id]);
        ...
        const now = new Date().toISOString();
        await this.db.run('UPDATE reservasis SET status = ?, check_in_time = ?, updated_at = ? WHERE id = ? AND maker_id = ?', ['aktif', now, now, id, user.maker_id]);
        return {
            success: true,
    ...
    async checkOut(user, id) {
       ...
       WHERE r.id = ? AND r.maker_id = ?`, [id, user.maker_id]);
        ...
        const now = new Date().toISOString();
        await this.db.run('UPDATE reservasis SET status = ?, check_out_time = ?, updated_at = ? WHERE id = ? AND maker_id = ?', ['selesai', now, now, id, user.maker_id]);
        return {
            success: true,
    ...
    async createMember(user, body) {
        ...
        const existingUser = await this.db.get('SELECT id FROM users WHERE username = ? AND maker_id = ?', [username, user.maker_id]);
        if (existingUser) {
            throw new common_1.BadRequestException({
                success: false,
       ...
       VALUES (?, ?, ?, 'member', ?, ?)`, [user.maker_id, username, hashedPassword, now, now]);
        const memberRes = await this.db.run(`INSERT INTO members (user_id, nama_member, instansi, alamat, telp, foto, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
            userRes.lastID,
            nama_member,
            instansi || '-',
    ...
    async updateMember(user, id, body) {
        const member = await this.db.get(`SELECT m.*, u.maker_id FROM members m
       JOIN users u ON m.user_id = u.id
       WHERE m.id = ? AND u.maker_id = ?`, [id, user.maker_id]);
        if (!member) {
            throw new common_1.NotFoundException({
                success: false,
        ...
        const now = new Date().toISOString();
        await this.db.run(`UPDATE members SET
        ...
        foto = COALESCE(?, foto),
        updated_at = ?
       WHERE id = ?`, [nama_member ?? null, instansi ?? null, alamat ?? null, telp ?? null, foto ?? null, now, id]);
        return {
            success: true,
    ...
    async getMemberById(user, id) {
        const member = await this.db.get(`SELECT m.*, u.username, u.created_at as joined_at,
              COUNT(r.id) as total_reservasi,
              COALESCE(SUM(CASE WHEN r.status IN ('selesai', 'aktif', 'disetujui') THEN r.total_bayar ELSE 0 END), 0) as total_transaksi
       FROM members m
       JOIN users u ON m.user_id = u.id
       ...
       GROUP BY m.id`, [id, user.maker_id]);
        if (!member) {
            throw new common_1.NotFoundException({
                success: false,
                message: 'Member tidak ditemukan.',
    ...
    async deleteMember(user, id) {
        const member = await this.db.get(`SELECT m.*, u.id as user_id FROM members m
       JOIN users u ON m.user_id = u.id
       WHERE m.id = ? AND u.maker_id = ?`, [id, user.maker_id]);
        if (!member) {
            throw new common_1.NotFoundException({
    ...
    async getFinancialReport(user) {
        const summary = await this.db.get(`SELECT 
       ...
       FROM reservasis
       WHERE maker_id = ? AND status != 'dibatalkan'`, [user.maker_id]);
        const monthlyBreakdown = await this.db.all(`SELECT 
       ...
       ORDER BY bulan DESC`, [user.maker_id]);
        const spaceBreakdown = await this.db.all(`SELECT 
         s.id as space_id,
       ...
       WHERE s.maker_id = ?
       GROUP BY s.id
       ORDER BY total_pendapatan DESC`, [user.maker_id]);
        return {
            success: true,
            summary: summary || {
                total_transaksi: 0,
    ...
    async getMonthlyReport(user, queryParams) {
        const now = new Date();
       ...
       FROM reservasis
       WHERE maker_id = ? AND strftime('%Y-%m', tanggal_reservasi) = ? AND status != 'dibatalkan'`, [user.maker_id, targetPeriod]);
        const dailyRows = await this.db.all(`SELECT 
       ...
       ORDER BY day_num ASC`, [user.maker_id, targetPeriod]);
        ...
        const categoryRows = await this.db.all(`SELECT 
       ...
       ORDER BY total_pendapatan DESC`, [user.maker_id, targetPeriod]);
    ...
    async getVisitorReport(user) {
       ...
       FROM reservasis
       WHERE maker_id = ? AND status != 'dibatalkan'`, [user.maker_id]);
        const dailyBreakdown = await this.db.all(`SELECT 
         tanggal_reservasi as tanggal,
