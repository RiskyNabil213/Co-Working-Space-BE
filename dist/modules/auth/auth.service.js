...
let AuthService = class AuthService {
    ...
    async registerMember(makerId, body) {
        ...
        const existing = await this.db.get('SELECT id FROM users WHERE username = ? AND maker_id = ?', [username, makerId]);
        if (existing) {
            throw new common_1.BadRequestException({
                success: false,
       ...
       VALUES (?, ?, ?, 'member', ?, ?)`, [makerId, username, hashedPassword, now, now]);
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [userId, nama_member, instansi || null, alamat || null, telp, foto || null, now, now]);
    ...
    async registerAdminSpace(makerId, body) {
        const { username, password, nama_coworking, nama_pemilik, alamat, telp, deskripsi_fasilitas, } = body;
    ...
    async login(makerId, body) {
        ...
        let user = await this.db.get(`SELECT u.*,
       ...
       WHERE (LOWER(u.username) = LOWER(?) OR LOWER(m.nama_member) = LOWER(?)) AND u.maker_id = ?`, [username, username, makerId]);
        if (!user) {
            user = await this.db.get(`SELECT u.*,
                m.id as member_id, m.nama_member, m.instansi, m.alamat as member_alamat, m.telp as member_telp, m.foto,
                so.id as owner_id, so.nama_coworking, so.nama_pemilik, so.alamat as owner_alamat, so.telp as owner_telp, so.deskripsi_fasilitas
         FROM users u
         LEFT JOIN members m ON u.id = m.user_id
         LEFT JOIN space_owners so ON u.id = so.user_id
         WHERE (LOWER(u.username) = LOWER(?) OR LOWER(m.nama_member) = LOWER(?))`, [username, username]);
        }
        if (!user) {
            const maker = await this.db.get('SELECT * FROM makers WHERE LOWER(username) = LOWER(?) OR LOWER(email) = LOWER(?)', [username, username]);
            if (maker) {
                ...
                const now = new Date().toISOString();
                let existingUser = await this.db.get('SELECT * FROM users WHERE maker_id = ? AND username = ?', [maker.id, maker.username]);
                let userId = existingUser?.id;
                if (!existingUser) {
                    const userRes = await this.db.run(`INSERT INTO users (maker_id, username, password, role, created_at, updated_at)
             VALUES (?, ?, ?, 'admin_space', ?, ?)`, [maker.id, maker.username, maker.password, now, now]);
                    userId = userRes.lastID;
                }
                let existingOwner = await this.db.get('SELECT * FROM space_owners WHERE user_id = ?', [userId]);
    ...
    async getProfile(user) {
        const refreshed = await this.db.get(`SELECT u.id, u.maker_id, u.username, u.role,
    ...
    async updateProfile(user, body) {
        ...
        if (username && username.trim() !== user.username) {
            const existing = await this.db.get('SELECT id FROM users WHERE username = ? AND maker_id = ? AND id != ?', [username.trim(), user.maker_id, user.id]);
            if (existing) {
                throw new common_1.BadRequestException({
                    success: false,
            ...
            await this.db.run('UPDATE users SET username = ?, updated_at = ? WHERE id = ?', [username.trim(), now, user.id]);
        }
        if (password && password.trim().length > 0) {
            const hashedPassword = await bcrypt.hash(password.trim(), 10);
            await this.db.run('UPDATE users SET password = ?, updated_at = ? WHERE id = ?', [hashedPassword, now, user.id]);
        }
        if (user.role === 'member' || user.member_id) {
            const member = await this.db.get('SELECT id FROM members WHERE user_id = ? OR id = ?', [user.id, user.member_id || 0]);
            if (member) {
           ...
           WHERE id = ?`, [
                ...
                await this.db.run(`INSERT INTO members (user_id, nama_member, instansi, alamat, telp, foto, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                    user.id,
        ...
        const updatedUser = await this.db.get(`SELECT u.id, u.maker_id, u.username, u.role,
              m.id as member_id, m.nama_member, m.instansi, m.alamat, m.telp, m.foto,
              so.id as owner_id, so.nama_coworking, so.nama_pemilik, so.alamat as owner_alamat, so.telp as owner_telp
       FROM users u
       LEFT JOIN members m ON u.id = m.user_id
