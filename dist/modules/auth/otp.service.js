...
let OtpService = OtpService_1 = class OtpService {
    ...
    async sendOtp(email) {
        ...
        if (transporter) {
          ...
          `,
                ...
                throw new common_1.BadRequestException(`Gagal mengirimkan email OTP: ${err.message}`);
            }
        }
        else {
    ...
    async verifyOtp(email, inputOtp) {
        if (!email || !inputOtp) {
            throw new common_1.BadRequestException('Email dan kode OTP wajib diisi.');
        }
        const normalizedEmail = email.trim().toLowerCase();
        const cleanOtp = inputOtp.trim();
        const record = this.otpStore.get(normalizedEmail);
        if (!record) {
            throw new common_1.BadRequestException('Kode OTP belum diminta atau sudah hangus. Silakan minta kode baru.');
        }
        if (new Date() > record.expiresAt) {
            this.otpStore.delete(normalizedEmail);
            throw new common_1.BadRequestException('Kode OTP telah kedaluwarsa (lebih dari 5 menit). Silakan minta kode baru.');
        }
        if (record.attempts >= 3) {
            this.otpStore.delete(normalizedEmail);
            throw new common_1.UnauthorizedException('Terlalu banyak percobaan salah. Silakan minta kode OTP baru.');
        }
        ...
        if (!isMatch) {
            ...
            throw new common_1.BadRequestException(`Kode OTP salah. Sisa kesempatan mencoba: ${sisa} kali.`);
        }
        this.otpStore.delete(normalizedEmail);
